...
header = ["name","formatted_address","formatted_phone_number","website","opening_hours","rating","latitude","longitude"]

# Create a DataFrame for the header and write it to the CSV file
df_header = pd.DataFrame(columns=header)
filename = "Datasets/facility_info3.csv"
...
for street,name,city,state in zip(street_list, facility_list,city_list,state_list):
	...
	try:
		row = [web_info['result']['name'] if "name" in web_info['result'] else 'NaA',
			...
			web_info['result']['geometry']['location']['lat'] if "geometry" in web_info['result'] else 'NaA',
			web_info['result']['geometry']['location']['lng'] if "geometry" in web_info['result'] else 'NaA',
			]
	except:
		row = ['NaA','NaA','NaA','NaA','NaA','NaA','NaA','NaA']
